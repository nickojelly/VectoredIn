...
# Taken from https://github.com/RyanLiGod/hnsw-python/blob/master/hnsw.py and adjusted to needs
...
class HNSW(object):
    ...
    def vectorized_distance_select_(self, x, ys, func, adj):
        return [func(x, y,adj) for y in ys]

    def __init__(self, distance_type,df, m=5, ef=200, m0=None, heuristic=True, vectorized=False):
        self.data = []
        self.df = df
        if distance_type == "l2":
            # l2 distance
    ...
    def load_from_dict(self, data_dict):
        ...
        self._enter_point = 0
        self._graphs = [data_dict[k] for k in self.data]
    ...
    def serach_along_axis(self, q, k=None, ef=None, n=5):
        """
        This function performs a search along the axis of a given query vector, returning k closest vectors at n intervals along the axis.
        """
        ...
        # Search for the vectors with the minimum distance to the query vector
        min_vecotrs = self.search(q,k=k,ef=ef,adj=0)
        min_dist = min_vecotrs[0][1]
        full_vector_list[0] = min_vecotrs

        # Search for the vectors with the maximum distance to the query vector
        max_vectors = self.search(q,k=k,ef=ef,adj=2)
        max_dist = self.vectorized_distance(q,[self.data[x[0]] for x in max_vectors])
        full_vector_list[n-1] = list(zip([x[0] for x in max_vectors],max_dist))

        # Calculate the range and interval of the distances
        distance_range = max_dist[0] - min_dist
        interval = distance_range/n

        # Perform a search at each interval along the axis
        ...
        # Remap the indices to UUIDs and create a dictionary of vectors
        remapped_dict = {} 
        vector_dict = {}
